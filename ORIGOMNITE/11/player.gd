...
var HP = 30
var Stam = 20
@onready var barra_de_vida = $"../CanvasLayer/Barras/Health"
@onready var barra_de_stam = $"../CanvasLayer/Barras/Mana"
...
func _input(event):
	...
	if event is InputEventJoypadMotion:
		spring_arm_pivot.rotate_y(JOY_AXIS_RIGHT_X * .005)
		spring_arm.rotate_x(JOY_AXIS_RIGHT_Y * .005)
		spring_arm.rotation.x = clamp(spring_arm.rotation.x, -PI/4, PI/4)
...
func ataquelegal():
	...
	for junin_inim in enemies6:
		if junin_inim.has_method("doiespecial"):
				junin_inim.doiespecial()
