...
@onready var Animação = $AnimationPlayer

#variavel da barra de vida
@onready var progress_life = $NPC/SubViewport/ProgressBar
...
func _process (delta):
	...
	if HP > 0:
		...
		if global_position.distance_to(target.global_position) < CHASE_RANGE:
			...
			if not anim_rodando:
				Animação.play("Global/caminharJunin")
		else:
			Animação.play("Global/juninRespirando")
		...
		if global_position.distance_to(target.global_position) < ATK_RANGE and not anim_rodando:
			...
			match a:
				1:
					Animação.play("Global/ataqueJunin01")
					a = a+1
				2:
					Animação.play("Global/ataqueJunin02")
					a = 1
...
func morte():
	Animação.play("Global/receberNocaute")

func _on_animation_player_animation_started(anim_name):
	if anim_name == "Global/ataqueJunin01" or anim_name == "Global/ataqueJunin02" or anim_name == "Global/Respirando" or anim_name == "Global/receberAtaque" or anim_name == "Global/receberAtaque_02":
		anim_rodando = true
...
func _on_animation_player_animation_finished(anim_name):
	...
	if anim_name == "Global/receberNocaute":
		queue_free()
		
func doinormal():
	HP = HP - 10
	Animação.play("Global/receberAtaque")
	SPEED = 0.0
	print("Parou")
	await(1)
	SPEED = 150.0
...
func doiespecial():
	HP = HP - 15
	Animação.play("Global/receberAtaque_02")
	SPEED = 0.0
	print("Parou2")
	await(1)
	SPEED = 150.0
